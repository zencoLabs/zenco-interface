...
class EIP6963ProviderManager {
  ...
  constructor() {
    global?.window?.addEventListener(EIP6963Event.ANNOUNCE_PROVIDER, this.onAnnounceProvider.bind(this) as EventListener)
    global?.window?.dispatchEvent(new Event(EIP6963Event.REQUEST_PROVIDER))
  }
...
export function subscribeToProviderMap(listener: () => void): () => void {
  EIP6963_PROVIDER_MANAGER.listeners.add(listener)
  return () => EIP6963_PROVIDER_MANAGER.listeners.delete(listener)
}

export function getProviderMapSnapshot(): InjectedProviderMap {
